########################################################
#
#   Confirmatory Factor Analysis
#   6 factor with 20 observed variables (9,12,23,24 제외)
#    with dependent variables
#    
##########################################################

library(tidyverse)
library(lavaan)

#  Data set prepration

load("kiosk_clean.rda")
sq_per <- kiosk_clean[, seq(from=7, to=53, by=2)]
ipa_sat <- kiosk_clean[, seq(from=113, to=159, by=2)]

sq_per <- cbind(sq_per, kiosk_clean[,160:163])
for(i in 1:24) names(sq_per)[i] <- paste("x",i,sep="")
for( i in 1:4) names(sq_per)[24+i] <- paste("y",i,sep="")

ipa_sat <- cbind(ipa_sat, kiosk_clean[,160:163])
for(i in 1:24) names(ipa_sat)[i] <- paste("x",i,sep="")
for( i in 1:4) names(ipa_sat)[24+i] <- paste("y",i,sep="")


#  CFA for sq_per (x9, x12, x23, x24 제외)

cfa1 <-"
# Factor loadings (LAMDA)
F1 =~ x4 + x5 + x6 + x22              # 유형성 (tangible)
F2 =~ x19 + x20 + x21                 # 보안성 (security)
F3 =~ x10 + x11 + x18                 # 절약성 (economy)
F4 =~ x13 + x14 + x15 + x16 + x17     # 편리성 (convenience)
F5 =~ x1 + x2 + x3                    # 유용성 (reliability)
F6 =~ x7 + x8                         # 통제성 (control)
# Covariance Between Latent variables (PHI)
F1 ~~ F1; F2 ~~ F2; F3 ~~ F3; F4 ~~ F4; F5 ~~ F5; F6 ~~ F6;
F1 ~~ F2; F1 ~~ F3; F1 ~~ F4; F1 ~~ F5; F1 ~~ F6; F2 ~~ F3;
F2 ~~ F4; F2 ~~ F5; F2 ~~ F6; F3 ~~ F4; F3 ~~ F5; F3 ~~ F6;
F4 ~~ F5; F4 ~~ F6; F5 ~~ F6
# Variances of observed variables (THETA)
x1 ~~ x1; x2 ~~ x2; x3 ~~ x3; x4 ~~ x4; x5 ~~ x5;
x6 ~~ x6; x7 ~~ x7; x8 ~~ x8; x10 ~~ x10;
x11 ~~ x11; x13 ~~ x13; x14 ~~ x14; x15 ~~ x15; x16 ~~ x16; 
x17 ~~ x17; x18 ~~ x18; x19 ~~ x19; x20 ~~ x20;
x21 ~~ x21; x22 ~~ x22
"

#
#  add resultant questions total satistfaction and re-use intentions in CFA

# add one Latent dependent variable R

cfa2 <- paste(cfa1,
"# add one latent dependent construct R  loadings (LAMDA)
R =~ y1 + y2 + y3 + y4
#  add Covariance Bewteen Latent variables (PHI)
R ~~ R; R ~~ F1; R ~~ F2; R ~~ F3; R ~~ F4; R ~~ F5; R ~~ F6
#  add Variance of observed variables (THETA)
y1 ~~ y1; y2 ~~ y2; y3 ~~ y3; y4 ~~ y4"
)

#  add two  dependent constructs R1, R2

cfa3 <- paste(cfa1,
"# add final two latent dependent variables R1, R2 (LAMDA)
R1 =~ y1 + y2 
R2 =~ y3 + y4
# add Covariance Between Latent Variables (PHI)
R1 ~~ F1; R1 ~~ F2; R1 ~~ F3; R1 ~~ F4; R1 ~~ F5; R1 ~~ F6
R2 ~~ F1; R2 ~~ F2; R2 ~~ F3; R2 ~~ F4; R2 ~~ F5; R2 ~~ F6
R1 ~~ R1; R2 ~~ R2; R1 ~~ R2
# add variance of observed variables (THETA)
y1 ~~ y1; y2 ~~ y2; y3 ~~ y3; y4 ~~ y4"
)

#
#  Apply for the ipa_sat data set as it is the best model
#

#   cfa2: with one dependent latent construct R

sat_cfa2 <- sem(cfa2, data=ipa_sat)
summary(sat_cfa2, fit.measures=T, standardized=T)

# compute CR(composite reliability) and AVE(average variance extected) for factors

cr_ave <- function(loadings, thetas) {
  ss_l2 <- sum(loadings)^2
  ss_lsq <- sum(loadings^2)
  ss_thetas <- sum(thetas)
  CR <- ss_l2/(ss_l2+ss_thetas)
  AVE <- ss_lsq/(ss_lsq+ss_thetas)
  results <- cbind(CR, AVE)
  colnames(results) <- c("CR:composite reliability", "AVE: average variance extracted")
  results
}

est <- standardizedsolution(sat_cfa2)
F1_loadings <- est[1:4, 4]
F1_thetas <- est[c(45,46,47,61),4]
cr_ave(F1_loadings, F1_thetas)
F2_loadings <- est[5:7, 4]
F2_thetas <- est[c(58:60), 4]
cr_ave(F2_loadings, F2_thetas)
F3_loadings <- est[8:10, 4]
F3_thetas <- est[c(50,51,57), 4]
cr_ave(F3_loadings, F3_thetas)
F4_loadings <- est[11:15, 4]
F4_thetas <- est[52:56, 4]
cr_ave(F4_loadings, F4_thetas)
F5_loadings <- est[16:18, 4]
F5_thetas <- est[42:44, 4]
cr_ave(F5_loadings, F5_thetas)
F6_loadings <- est[19:20, 4]
F6_thetas <- est[48:49, 4]
cr_ave(F6_loadings, F6_thetas)
R_loadings <- est[62:65,4]
R_thetas <- est[73:76,4]
cr_ave(R_loadings, R_thetas)

# cfa3: with two dependent latent constrcts R1 R2

sat_cfa3 <- sem(cfa3, data=ipa_sat)
summary(sat_cfa3, fit.measures=T, standardized=T)

est <- standardizedsolution(sat_cfa3)
F1_loadings <- est[1:4, 4]
F1_thetas <- est[c(45,46,47,61),4]
cr_ave(F1_loadings, F1_thetas)
F2_loadings <- est[5:7, 4]
F2_thetas <- est[c(58:60), 4]
cr_ave(F2_loadings, F2_thetas)
F3_loadings <- est[8:10, 4]
F3_thetas <- est[c(50,51,57), 4]
cr_ave(F3_loadings, F3_thetas)
F4_loadings <- est[11:15, 4]
F4_thetas <- est[52:56, 4]
cr_ave(F4_loadings, F4_thetas)
F5_loadings <- est[16:18, 4]
F5_thetas <- est[42:44, 4]
cr_ave(F5_loadings, F5_thetas)
F6_loadings <- est[19:20, 4]
F6_thetas <- est[48:49, 4]
cr_ave(F6_loadings, F6_thetas)
R1_loadings <- est[62:63,4]
R1_thetas <- est[81:82,4]
cr_ave(R1_loadings, R1_thetas)
R2_loadings <- est[64:65,4]
R2_thetas <- est[83:84,4]
cr_ave(R2_loadings, R2_thetas)



