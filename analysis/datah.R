...
name <- paste("sq",1:24, sep="")
name1 <- paste(name,"_1", sep="")
name2 <- paste(name, "_2", sep="")
name_sq <- vector()
for(i in 1:length(name)){
    name_sq[2*i-1] <- name1[i]  
    name_sq[2*i] <- name2[i] 
}
name <- paste("kano",1:29, sep="")
name1 <- paste(name,"_1", sep="")
name2 <- paste(name, "_2", sep="")
name_kano <- vector()
for(i in 1:length(name)){
  name_kano[2*i-1] <- name1[i]  
  name_kano[2*i] <- name2[i] 
}
name <- paste("ipa",1:24, sep="")
name1 <- paste(name,"_1", sep="")
name2 <- paste(name, "_2", sep="")
name_ipa <- vector()
for(i in 1:length(name)){
  name_ipa[2*i-1] <- name1[i]  
  name_ipa[2*i] <- name2[i] 
}
...
#  이상 데이타 검토 (비정상적 obervations)

round(sapply(sq_exp, mean), 2)
round(sapply(sq_per, mean), 2)
round(sapply(sq_value, mean), 2)
apply(as.matrix(sq_per), 1, table) # per 데이터가 지나치게 몰린 경우 

round(sapply(ipa_imp, mean), 2)
round(sapply(ipa_sat, mean), 2)
apply(as.matrix(ipa_sat), 1, table)  # sat 데이터가 지나치게 몰린 경우 


round(apply(as.matrix(sq_exp),1, mean), 1)
mean(apply(as.matrix(sq_exp),1, mean))
sd(apply(as.matrix(sq_exp), 1, mean))
which(apply(as.matrix(sq_exp), 1, mean) <=4.5) # exp값이 지나치게 적은 경우 

round(apply(as.matrix(ipa_imp), 1, mean), 1)
mean(apply(as.matrix(ipa_imp), 1, mean))
sd(apply(as.matrix(ipa_imp), 1, mean))
which(apply(as.matrix(ipa_imp), 1, mean) <= 4.5) # imp 값이 지나치게 적은 경우 
